...
[監査概要](../validation/artifacts/candidate-packaging-20260911/README.md)と[機械可読summary](../validation/artifacts/candidate-packaging-20260911/summary.json)に、正確なSHA・環境・ハッシュ・由来を保存。
...
- 公開4者のwheel/sdist：`review-artifacts/20260911/dist/`（gitignore、ハッシュは上記summaryに記録）。
- Fのwheel/sdist：`sashimi-f/artifacts/release-preparation-20260911/dist/`（private、gitignore）。
- 全5者の全数値・テスト証拠：`sashimi-f/artifacts/release-preparation-20260911/full-evidence/`（private、gitignore）。
- 元の作業環境とログ：`/tmp/sashimi-migration-20260910/rc-packaging-20260911/`、`/tmp/sashimi-migration-20260910/rc-packaging-f-testfix-20260911/`。
...
以下は一時effective manifestで検証した候補集合であり、恒久的な互換性集合を新設するものではない。canonicalは引き続きrootの`compatibility.toml`。この証拠の基準親commitは `7d4e244f61929b3b670385bb20da9d816af4bffe`。

| Repo | ローカルbranch | 検証した完全SHA |
| --- | --- | --- |
| itamae | `codex/release-candidate-20260910` | `dfa083d0d46181c376947ac7b2da829facaf2e8b` |
...
| sashimi-f | `codex/f-installed-regression-20260911` | `dc1b30574bf732f195b0777b6e64981d858d4c92` |

全child候補はorigin/itamae-migrationへ統合済み。 coreの候補artifactはtopic head `dfa083d0d46181c376947ac7b2da829facaf2e8b`由来で、umbrellaのmerge headは `6d8ee62b65793df9b799977cbf1859b5233d4059`。Git treeはともに `1bf4f4bd4bdf74228041b711d4651056d66bedd1`だが、source identityは異なる。現artifactをmerge SHA由来と付け替えず、最終採用SHAを揃える際は該当artifactを作り直す。親の作業branchは `codex/migration-release-20260910`。親のcommitted gitlinkとmanifestは旧確認済み集合のままで一致している。作業ツリーの5つのsubmoduleは上表の候補へ進めているため、親でmodifiedと表示されるのは意図した状態。**最終科学判断が済む前にgitlinksだけstageしない。**
...
4. private Fの最終CI設定を更新し、必要なchild CIへ合格させmigrationへ統合する。新SHAのF artifactと影響範囲を再検証。公開4者が不変ならハッシュ確認したartifactを再利用する。
