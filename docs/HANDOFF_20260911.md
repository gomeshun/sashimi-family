...
2026-09-11。**候補実装・科学検証・配布物検証は完了。親への記録後、同じ親SHAによる公開/privateの最終CI結果を確認する段階。**
...
候補検証後に、`compatibility.toml`と5 gitlinks、準備済みの公開family CIを一つの親migration commitへ記録する。committed HEAD検証後、公開family CIとprivate F workflowへ同じ親SHAを渡す。private側は`family_mode=promoted`を指定し、overrideが空であることを取得済みartifactで確認する。この結果は検証後に追記する。
